# Technical Architecture & Visual Engineering Blueprint

**Project**: UNIKOM KKN Berdampak 2026 — Kecamatan Coblong, Kota Bandung  
**Document ID**: `DOC-02-TECHNICAL-BLUEPRINT`  
**Specification Type**: WebGL 1.0 Rendering Pipeline, Photometric Color Science, & 6-Act Penta Helix Layout Architecture  
**Reference Baseline**: Live Citrix Red Bull Racing Forensic Audit (`https://thenewmobileworkforce.imm-g-prod.com/`) & Sustainable Web Design Forensic Audit (`https://sustainablewebdesign.org/`)  
**Evidence Standard**: **`[VERIFIED]`** (All specifications derived directly from quantitative deconvolution of source stylesheets and scripts)  

---

## 1. Complete Application Architecture Diagram & Reactivity Isolation

Sistem ini menggabungkan *reactive DOM presentation layer* (Vue 3 / Nuxt 3 SFC — 25 Komponen SFC) dengan *raw WebGL 1.0 multi-canvas render engine* (123 Geometri Sub-Mesh 3D + 26 Composables Shader) dan *Web Audio API manager* `[VERIFIED]`:

### 1.1 Isolasi Reaktivitas Vue 3 dari Objek Heavy WebGL `[VERIFIED]`
...
```mermaid
graph TD
    subgraph HTML_DOM_Layer["HTML5 DOM Layer (25 SFC UI Components)"]
        AppVue["app.vue (Root Shell, Module 414 / 387)"]
        AppHeader["components/AppHeader.vue (Top Navigation, Module 387)"]
        AppNav["components/AppNav.vue (Act Dropdown, Module 387)"]
        AppLoader["components/AppLoader.vue (Preloader Badge, Module 387)"]
        AppScenes["components/AppScenes.vue (WebGL Host Canvas, Module 387)"]
        AppSlideshow["components/AppSlideshow.vue (Master Slideshow, Module 387)"]
        AppSlide["components/AppSlide.vue (Act Overlay & Typography, Module 407)"]
        AppSlideDescription["components/AppSlideDescription.vue (Slide Text Narrative, Module 391)"]
        AppSlideBtnMore["components/AppSlideBtnMore.vue (Expandable Info Button, Module 406)"]
        AppSlideBottom["components/AppSlideBottom.vue (Action Triggers Bar, Module 406)"]
        AppSlideBottom1_5["AppSlideBottom1..5.vue (Act Triggers 1..5, Module 403)"]
        AppCounter["components/AppCounter.vue (Progress Counter 01/06, Module 400)"]
        AppKeyPoint["components/AppKeyPoint.vue (Highlight Badge, Module 406)"]
        AppMagnet["components/AppMagnet.vue (Hotspot Target Pins, Module 403)"]
        AppModalDetail["components/AppModalDetail.vue (Slide-In Briefing Drawer, Module 387)"]
        AppSoundManager["components/AppSoundManager.vue (Web Audio Manager, Module 387)"]
        CoreBtnEmitter["components/CoreBtnEmitter.vue (Event Ring Emitter, Module 403)"]
        CoreDrag["components/CoreDrag.vue (Touch Drag Controller, Module 395)"]
        CoreVideoControls["components/CoreVideoControls.vue (Video Player UI, Module 394)"]
        CoreVideoYoutube["components/CoreVideoYoutube.vue (YouTube Frame, Module 394)"]
        AppPage404["components/AppPage404.vue (404 Fallback Page, Module 387)"]
    end

    subgraph WebGL_Submesh_Layer["WebGL 3D Pipeline Layer (123 3D Sub-Mesh Components & 26 Composables)"]
        UseSceneLoader["composables/useSceneLoader.ts (Scene Lifecycle, Module 345)"]
        UseHalftoneCanvas["composables/useHalftoneCanvas.ts (WebGL Engine, Module 356)"]
        ...
        GLSLTransition["shaders/transition.frag (5-Column Speed Distortion Shader)"]
    end
...
## 2. Complete Multi-Layer Component Taxonomy `[VERIFIED]`

Aplikasi terdiri dari **416 Webpack modules** yang mengendalikan **174 total komponen terdaftar** melintasi 3 tingkatan taksonomi `[VERIFIED]`:

### 2.1 Matriks 25 Komponen Antarmuka Utama (Top-Level SFC UI Components)

| # | Subsystem SFC Component | Webpack Module ID | Production File Path | Subsystem Purpose & Responsibilities |
| :--- | :--- | :--- | :--- | :--- |
| 1 | **Root Shell (`app.vue`)** | `414 / 387` | `app.vue` (`/application/index.vue`) | Master layout container, global stacking context manager (`z-index: 1..3`), modal drawer mounting. |
| 2 | **Top Header Bar (`AppHeader`)** | `387` | `components/AppHeader.vue` | Wordmark logo UNIKOM top-left, sound toggle top-right, level back button (`BACK TO MAP ▲`). |
| 3 | **Category Nav (`AppNav`)** | `387` | `components/AppNav.vue` | Dropdown selector Act (`COBLONG DISTRICT ▾`) & toggle `LEVEL 1 ↔ LEVEL 2`. |
| 4 | **Preloader Badge (`AppLoader`)** | `387` | `components/AppLoader.vue` | Badge co-branding (`UNIKOM x KECAMATAN COBLONG`), animasi SVG boundary stroke, 3D plane dissolve. |
| 5 | **WebGL Scenes Host (`AppScenes`)** | `387` | `components/AppScenes.vue` | Host canvas WebGL 1.0 multi-canvas, mengelola resize event & pixel ratio. |
| 6 | **Master Slideshow (`AppSlideshow`)** | `387` | `components/AppSlideshow.vue` | GSAP ScrollTrigger pinning across 6 Acts, keyboard shortcuts (`Panah`, `ESC`, `M`, `L`). |
| 7 | **Slide Container (`AppSlide`)** | `407` | `components/AppSlide.vue` | Wrapper konten slide tiap Act, pemicu overlay video interaktif dan animasi kemunculan teks. |
| 8 | **Slide Narrative (`AppSlideDescription`)** | `391` | `components/AppSlideDescription.vue` | Mengelola narasi deskripsi slide untuk tampilan desktop dan seluler responsif. |
| 9 | **Expandable Button (`AppSlideBtnMore`)** | `406` | `components/AppSlideBtnMore.vue` | Tombol interaktif untuk memperluas penjelasan narasi teknis program KKN. |
| 10 | **Slide Action Bar (`AppSlideBottom`)** | `406` | `components/AppSlideBottom.vue` | Kontainer kontrol bawah slide, mengelola panah navigasi `← PREV`/`NEXT →` dan progress bar. |
| 11 | **Act 1 Trigger (`AppSlideBottom1`)** | `403` | `components/AppSlideBottom1.vue` | Pemicu aksi khusus Act I (Hero Landing & Overview). |
| 12 | **Act 2 Trigger (`AppSlideBottom2`)** | `403` | `components/AppSlideBottom2.vue` | Pemicu aksi khusus Act II (Government Helix - SDGs Goal 11 Kota Bandung). |
| 13 | **Act 3 Trigger (`AppSlideBottom3`)** | `403` | `components/AppSlideBottom3.vue` | Pemicu aksi khusus Act III (Academia Helix - Tri Dharma Perguruan Tinggi). |
| 14 | **Act 4 Trigger (`AppSlideBottom4`)** | `403` | `components/AppSlideBottom4.vue` | Pemicu aksi khusus Act IV (Society Helix - Pelaksanaan Lapangan 6 Kelurahan). |
| 15 | **Act 5 Trigger (`AppSlideBottom5`)** | `403` | `components/AppSlideBottom5.vue` | Pemicu aksi khusus Act V (Business Helix - CSR & Sinergi Industri). |
| 16 | **Progress Counter (`AppCounter`)** | `400` | `components/AppCounter.vue` | Penanda nomor urut langkah aktif (`01 / 06` s/d `06 / 06`). |
| 17 | **Highlight Badge (`AppKeyPoint`)** | `406` | `components/AppKeyPoint.vue` | Lencana sorotan poin kunci indikator keberhasilan program. |
| 18 | **Hotspot Pins (`AppMagnet`)** | `403` | `components/AppMagnet.vue` | Anchored pins (`+`) di atas Kelurahan; hover inline badge + pulse ring (`#FF2A6D`), click opens detail drawer. |
| 19 | **Program Modal (`AppModalDetail`)** | `387` | `components/AppModalDetail.vue` | Slide-in drawer dengan 1:1 SWD Layout memuat murni konten SDG 11 Kota Bandung. |
| 20 | **Audio Manager (`AppSoundManager`)** | `387` | `components/AppSoundManager.vue` | Dual-track ambient audio (`ambiant-level-1.mp3` vs `level-2.mp3`), 9 sound stems/SFX, 4 equalizer wave bars. |
| 21 | **Event Emitter (`CoreBtnEmitter`)** | `403` | `components/CoreBtnEmitter.vue` | Emiter animasi lingkaran tombol pemicu interaktif `EXPLORE PROGRAM ➔` (`73px × 73px`). |
| 22 | **Touch Controller (`CoreDrag`)** | `395` | `components/CoreDrag.vue` | Pengelola gestur usapan (*drag/swipe*) kursor dan layar sentuh untuk navigasi lanskap. |
| 23 | **Video Controls (`CoreVideoControls`)** | `394` | `components/CoreVideoControls.vue` | Kontrol pemutar media video (Play/Pause, Mute, Fullscreen, Progress Bar). |
| 24 | **YouTube Frame (`CoreVideoYoutube`)** | `394` | `components/CoreVideoYoutube.vue` | Bingkai penyemat video dokumentasi KKN dari YouTube. |
| 25 | **Fallback Page (`AppPage404`)** | `387` | `components/AppPage404.vue` | Halaman fallback rute tak ditemukan dengan visualisasi sirkuit modus gelap (`.c-404`). |

### 2.2 Taksonomi 123 Geometri Sub-Mesh 3D (WebGL Scene & Object Components) `[VERIFIED]`
...
### 2.3 Taksonomi 26 Composable Hooks Shader & WebGL `[VERIFIED]`
...
## 3. Precision UI Measurements, Spacing, Stacking & Accessibility System `[VERIFIED]`

Ekstraksi kuantitatif matematis dari `build.css` mengonfirmasi dimensi piksel dan rasio viewport presisi:

### 3.1 Dimensi Piksel Elemen Antarmuka Presisi (8 Rule CSS Presisi)

* **Primary Action Ring Button (`.t-icon--play-circle`)**: **`73px × 73px`** (Desktop) / **`60px × 60px`** (Tablet/Mobile).
* **Play Icon Triangle (`.t-icon--play`)**: **`7px × 9px`** di tengah tombol ring.
* **Tall Vertical Arrow Indicator (`.t-icon--arrow-tall`)**: **`5px × 85px`** (Desktop) / **`5px × 50px`** (Mobile).
* **Horizontal End Arrow (`.t-icon--arrow-x-end`)**: **`7px × 8px`** (Tombol `NEXT →` / `← PREV`).
...
### 3.2 Sistem Grid Viewport Rasio Presisi (`vh` / `vw`)

```css
/* Top Viewport Padding (Exact 1/6 Viewport Height) */
.u-pad-t-vh1of6 { padding-top: 16.666666666666668vh; }

/* Left Margin Alignment (Exact 2/8 Viewport Width = 25%) */
.u-marg-l-w2of8 { margin-left: 25%; }

/* Bottom Margin Spacing (Exact 2/10 Viewport Height = 20vh) */
...
### 3.3 Tipografi & Kuantifikasi Line Height

* **Primary Display Title (`.t-h1`)**: `line-height: 1.0` (Tanpa spasi vertikal ekstra).
* **Small Line Height (`.t-text-lh--sm`)**: `line-height: 1.1`.
* **Medium Line Height (`.t-text-lh--md`)**: `line-height: 1.15`.
* **Body Narrative (`.t-text--xxl`)**: `line-height: 1.6`.
...
### 3.5 Ergonomi Target Sentuh & Aksesibilitas Pembaca Layar (Screen Reader WCAG 2.1) `[VERIFIED]`
...
## 4. Responsive Breakpoints & GSAP Animation Easing Matrix `[VERIFIED]`

### 4.1 CSS Media Query Breakpoints (7 Breakpoints & Modifikator Suffix) `[VERIFIED]`
...
2. `only screen and (orientation:landscape) and (max-width:667px)` — Modus lanskap seluler.
3. `screen and (max-width:770px)` (`@sm`) — Modus potret tablet.
4. `screen and (max-height:800px)` — Layar laptop / viewport pendek.
5. `screen and (max-width:1024px)` (`@md`) — Modus lanskap tablet.
6. `screen and (max-width:1280px)` (`@lg`) — Layar desktop standar.
7. `screen and (min-width:1600px)` (`@xl`) — Layar ultra-wide display.

### 4.2 Matriks 10 GSAP Animation Easing Functions `[VERIFIED]`
...
* **`Power2.easeOut`**: Transisi kemunculan slide typography & overlay teks.
* **`Power2.easeInOut`**: Interpolasi scrubbing 5-Column Speed Distortion Transition Shader.
* **`Power2.easeIn`**: Animasi penutupan slide/modal.
* **`Power3.easeOut`**: Animasi hover scale pada action ring button & pulse magnet hotspot pin.
* **`Power3.easeInOut`**: Interpolasi pergerakan kursor kinesiologis dan tilt paralaks 3D.
* **`Power4.easeOut`**: Transisi 3D plane dissolve pada preloader (`AppLoader.vue`).
* **`Power4.easeIn`**: Transisi keluar dissolve preloader.
* **`Quad.easeInOut`**: Kurva crossfade gain audio ambient pada Web Audio API.
* **`TweenLite` / `gsap.timeline`**: Controller GSAP Scrollytelling.

### 4.3 Sinkronisasi Manipulasi DOM Antara GSAP dan Vue 3 `[VERIFIED]`

* **Pemisahan Wewenang Mutasi DOM**: GSAP mengendalikan mutasi gaya atribut inline secara langsung (`transform: translate3d(...)`, `opacity`), sedangkan Vue 3 mengelola struktur pohon elemen virtual DOM (`v-if`, `v-for`, teks konten).
...
## 5. Structural Matrix: 6-Act Penta Helix Navigation
...
### Act I — Hero Landing & Isomorphic Focal Geometry

* **Visual Composition**: Asymmetric Right-Aligned Focal Subject dengan Left-Bound Negative Space Void ($L \to 0.0$).
* **Geometry & Texture Projection**: 2D Photographic Texture mapped onto 3D Orthographic Plane Geometry (*Skeuomorphic Photo-Plane Shading*).
* **Parallax & Camera Dynamics**: 2.5D Frustum Camera Response driven by Normalized Pointer Vector $[-1, 1]$ dengan Exponential Lerp Damping ($\text{factor} = 0.05$).

### Act II — Government Helix: Institutional SDG 11 Alignment (Exclusively SDG 11 Kota Bandung Focus)

* **Visual Baseline (Citrix Trackside Parity)**: WebGL 1.0 scene dengan dark silhouette framing, continuous landscape terrain, & hotspot pins (`AppMagnet.vue`) + action trigger ring `EXPLORE PROGRAM ➔` (`73px × 73px`).
* **Post-Trigger Content Architecture (Sustainable Web Design Parity)**: Laci briefing (`AppModalDetail.vue`) mengadopsi 1:1 struktur layout `sustainablewebdesign.org` yang berfokus murni pada target & indikator SDG 11 Kota Bandung:
...
### Act III — Academia Helix: Higher Education Tri Dharma Integration

* **Structural Target**: Formal Integration of UNIKOM Community Empowerment & Academic Tri Dharma Directives.

### Act IV — Society Helix: Micro-Empowerment Field Execution

* **Structural Target**: Field Execution Specification across 6 Urban Kelurahans (Sadang Serang, Sekeloa, Dago, Cipaganti, Lebak Gede, Lebak Siliwangi).
* **Control Node**: Interactive Hotspot Target Pins (`AppMagnet.vue`) dengan Radar Pulse Halo (`#FF2A6D`).

### Act V — Business Helix: CSR & Industrial Synergy

* **Structural Target**: Corporate Social Responsibility (CSR) Framework & Public-Private Partnership Matrix.

### Act VI — Media Helix: Endless Masonry Stream Gallery

* **Visual Architecture**: Dark-Mode Asynchronous Masonry Grid dengan Endless Continuous Infinite Stream Matrix.

---

## 6. 3D Mesh Geometry, Lens Flare & Atmospheric Assets `[VERIFIED]`

Seluruh aset geometri mesh 3D dan tekstur dari situs referensi telah diunduh dan tersimpan di repositori lokal `docs/ref_assets/`:

### 6.1 Level 1 Mesh Geometry & Atmospheric Textures

* **Geometry Meshes (`json`)**:
  * `docs/ref_assets/assets/medias/3d/level-1/scene-0/main-mesh.json` (Act 1 Hero HQ Mesh, **19.15 KB** / 19.153 bytes)
  * `docs/ref_assets/assets/medias/3d/level-1/scene-1/main-mesh.json` (Act 2 Garage Mesh, **35.80 KB** / 35.802 bytes)
  * `docs/ref_assets/assets/medias/3d/level-1/scene-2/main-mesh.json` (Act 3 Trackside Mesh, **29.27 KB** / 29.274 bytes)
  * `docs/ref_assets/assets/medias/3d/level-1/scene-3/main-mesh.json` (Act 4 Telemetry Mesh, **19.79 KB** / 19.787 bytes)
  * `docs/ref_assets/assets/medias/3d/level-1/scene-4/main-mesh.json` (Act 5 Factory Mesh, **38.29 KB** / 38.293 bytes)
  * `docs/ref_assets/assets/medias/3d/level-1/scene-5/main-mesh.json` (Act 6 Sunset Mesh, **12.54 KB** / 12.544 bytes)
* **Atmospheric Lens Flares (`jpg`)**:
  * `docs/ref_assets/assets/medias/3d/level-1/flares/sun.jpg` (Sun Specular Flare, **42.19 KB**)
  * `docs/ref_assets/assets/medias/3d/level-1/flares/flare-1.jpg` (Anamorphic Lens Streak 1, **3.78 KB**)
  * `docs/ref_assets/assets/medias/3d/level-1/flares/flare-2.jpg` (Anamorphic Lens Streak 2, **3.47 KB**)

### 6.2 Level 2 Interactive 3D Inspector Meshes & MatCap Maps

* **3D Binary Geometry Meshes (`3ds`)**:
  * `docs/ref_assets/assets/medias/3d/level-2/car/f1-redbull-light.3ds` (**1.83 MB** / 1.832.531 bytes High-Poly Chassis)
  * `docs/ref_assets/assets/medias/3d/level-2/differential/differential.3ds` (**1.76 MB** / 1.764.550 bytes Differential Mesh)
* **MatCap Spherical Shading Maps (`jpg`)**:
  * `docs/ref_assets/assets/medias/3d/level-2/matcap/7.jpg` (Metallic Dark Slate Texture, **9.98 KB**)
  * `docs/ref_assets/assets/medias/3d/level-2/matcap/9.jpg` (High-Specular Cyan Texture, **21.00 KB**)
  * `docs/ref_assets/assets/medias/3d/level-2/matcap/11.jpg` (Warm Gold Specular Texture, **19.07 KB**)
  * `docs/ref_assets/assets/medias/3d/level-2/differential/shadow-map.jpg` (Pre-baked Ambient Occlusion, **87.83 KB**)
  * `docs/ref_assets/assets/medias/3d/level-2/red-glow.jpg` (Volumetric Thermal Engine Glow, **2.45 KB**)

### 6.3 Web Audio API Audio Stem Manifest `[VERIFIED]`

* `ambiant-level-1.mp3`: **954.35 KB** (Outdoor Field Ambient Audio Track)
* `ambiant-level-2.mp3`: **954.35 KB** (Level 2 Data Lab Ambient Track)
* `slide-01.mp3` .. `slide-05.mp3`: **213.34 KB – 235.42 KB** (5-Act Slide Audio Stems)
* `tick.mp3`, `tick-reverb.mp3`, `nav-over.mp3`, `play-video.mp3`: SFX Mikro-interaksi UI

### 6.4 Topologi Web Audio API & Penjadwalan Otomatisasi AudioParam `[VERIFIED]`
...
## 7. Manifest Complete 107 GLSL Uniform Shader Parameters `[VERIFIED]`
...
### 8.1 Complete Color Ledger System (15 Citrix Hex + 8 SWD Modal Briefing Hex) `[VERIFIED]`

| Token / Variabel Warna | Kode Hex | Vector RGB | Peruntukan Subsystem & Referensi Baseline |
| :--- | :--- | :--- | :--- |
...
| **`--swd-bg-cream`** | `#FCFBF5` | `rgb(252, 251, 245)` | Permukaan latar belakang laci briefing modal SWD |
| **`--swd-cyan-badge`** | `#00E8C5` | `rgb(0, 232, 197)` | Dual-tone header cyan wordmark & metric circle 11.a |
| **`--swd-lavender`** | `#AA9CFE` | `rgb(170, 156, 254)` | Dual-tone header dropdown selector Act |
| **`--swd-teardrop-orange`** | `#FF7200` | `rgb(255, 114, 0)` | Shape underlay teardrop orange hero banner |
| **`--swd-yellow-triangle`** | `#FFCC0A` | `rgb(255, 204, 10)` | Shape yellow triangle & metric shape 11.6 |
| **`--swd-red-rectangle`** | `#FF495C` | `rgb(255, 73, 92)` | Shape red rectangle urban challenge section |
| **`--swd-backdrop-glass`** | `rgba(51,51,51,.95)` | `rgb(51, 51, 51)` | Full-screen backdrop overlay glassmorphism modal |

### 8.2 Matriks Complete 8 Teknik Pencahayaan Fotometrik (Citrix Baseline) `[VERIFIED]`
...
### 9.1 Matriks 26 WebGL GPU Hardware Extensions (12 Queried Vendor Prefixes + 14 Engine Extensions) `[VERIFIED]`
...
### 9.2 Strategi Pemuatan Aset Multi-Tahap, Cache In-Memory & Pipeline Preload `[VERIFIED]`
...
### 9.3 Manifes Complete 6 Tipografi Ter-Preload (Citrix & Sustainable Web Design) `[VERIFIED]`

Sistem memuat 4 berkas font WOFF utama Citrix serta 2 rumpun tipografi Laci Modal Briefing SWD:
...
### 9.4 Rincian Beban Kinerja & Budget Payload (7.44 MB Gzip / 7.95 MB Uncompressed Total) `[VERIFIED]`
...
### 9.5 Lifecycle & Render Loop Throttling (Page Visibility API & GSAP Ticker) `[VERIFIED]`
...
### 9.6 GPU Context Loss & Explicit Resource Disposal Lifecycle `[VERIFIED]`
...
## 10. Verified 1:1 Sustainable Web Design Post-Trigger UI Architecture `[VERIFIED]`
...
### 10.2 Kalkulasi Matematik Modal Overlay Drawer (`.overlay`)

Tampilan slide-in modal drawer diatur oleh persamaan posisi `calc()` persis:
...
### 10.3 Matriks Kontainmen Spasial & Tipografi SWD

* **Container**: `max-width: 1360px`, `contain-left`: `750px`, `contain-center`: `700px`.
* **Tipografi**:
  * **`h1`**: `font-family: paralucent`, `font-size: 10rem` (100px), `line-height: 1`.
  * **`h2`**: `font-family: paralucent`, `font-size: 5.2rem` (52px), `line-height: 1.12`.
  * **`p`**: `font-family: "Courier New", Courier, monospace`, `font-size: 1.8rem` (28.8px) / `1.6rem` (25.6px).
  * **`.btn` (Pill Button Hitam)**: `border-radius: 20px`, `font-size: 1.7rem` (17px), `padding: 0.6rem 2.4rem`, `background-color: #000000`.
  * **`.category` (Pill Badge Filter)**: `border: 2px solid #000000`, `border-radius: 0.4rem` (4px), `padding: 0.5rem 1rem`, `margin-right: 10px`, `margin-bottom: 5px`, `font-size: 1.6rem`.
